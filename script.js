...
async function viewBalance(connection, account_id) {
...
async function lookup(account) {
  const inputAccountId = account; //document.querySelector('#account').value;
  ...
  try {
    ...
    // ownerAccountBalance = new BN(state.amount);
    // console.log("\t\towner's balance " + ownerAccountBalance);
    // total_result = total_result.add(new BN(ownerAccountBalance));
